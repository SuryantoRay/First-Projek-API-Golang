...
type Product struct {
	...
	Price float64 `json:"price" sql:"decimal(16,2)"`
}
...
func main() {
	db, err = gorm.Open("mysql", "root:@/goapi?charset=utf8&parseTime=True")
...
func handleRequest() {
	log.Println("Start the development server ar https://127.0.0.1:8080")
	...
	myRouter.HandleFunc("/api/products/{id}", delateProduct).Methods("DELETE")
...
func createProduct(w http.ResponseWriter, r *http.Request) {
	...
	res := Result{Code: 200, Data: product, Message: "Success created product"}
...
func delateProduct(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	productID := vars["id"]
	...
	res := Result{Code: 200, Message: "Success deleted product"}
	result, err := json.Marshal(res)
